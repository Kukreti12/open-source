...
def get_data_postgres():
    ...
    result = postgres_hook.get_records("SELECT taxitype, EXTRACT(YEAR FROM lastmonthprocessed) as year, EXTRACT(MONTH FROM lastmonthprocessed) as month FROM nyc")
    # Process the result and save it in a dictionary
    data_dict = {}
    for row in result:
        key = row[0]  # Assuming the first column is the key
        year = row[1]
        month = row[2]  # Assuming the second column is the value
